...
- 配置分两类：
  ...
  - 运行时配置：数据库里的 `system_config`，影响 WebDAV 开关、回收站保留期、版本数上限等业务行为
    也包括默认配额、团队归档保留期、CORS 行为、审计日志保留期和 Gravatar 基础地址这类业务配置
...
7. 确保存储策略组已种好；如果缺默认组会自动补，历史用户缺 `policy_group_id` 时也会回填到默认组
8. 确保默认运行时配置存在
9. 初始化运行时快照、驱动注册中心与缓存后端
10. 启动缩略图后台 worker
11. 注册 REST、健康检查、WebDAV 与前端路由
12. 启动后台周期任务：
    - 每小时：清理过期上传 session
