...
- `/swagger-ui` 与 `/api-docs/openapi.json`：仅 `debug` 构建注册
...
策略保存在数据库，而不是 `config.toml`。解析顺序是：
...
2. 通过 Basic 或 Bearer 做认证
3. 为本次请求构造带用户上下文的 `AsterDavFs`
4. 为本次请求构造数据库锁系统
5. 交给 `dav-server` 处理标准 WebDAV 方法
...
这些能力直接复用现有 `file_versions` 表。

## 前端交付方式

- 构建期：`build.rs` 会检查 `frontend-panel/dist`
- 若前端未构建，构建脚本会生成一个回退页，提醒先执行前端构建
