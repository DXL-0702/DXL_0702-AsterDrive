...
#[actix_web::test]
async fn test_cannot_delete_policy_referenced_by_legacy_avatar_upload() {
    use aster_drive::db::repository::user_profile_repo;
    use aster_drive::services::{auth_service, policy_service};
    use aster_drive::types::{AvatarSource, DriverType};
    use chrono::Utc;
    use sea_orm::Set;

    let state = common::setup().await;
    let avatar_policy_base_path = format!("/tmp/asterdrive-avatar-policy-{}", uuid::Uuid::new_v4());
    std::fs::create_dir_all(&avatar_policy_base_path).unwrap();
    let avatar_policy = policy_service::create(
        &state,
        policy_service::CreateStoragePolicyInput {
            name: "Legacy Avatar Policy".to_string(),
            connection: policy_service::StoragePolicyConnectionInput {
                driver_type: DriverType::Local,
                endpoint: String::new(),
                bucket: String::new(),
                access_key: String::new(),
                secret_key: String::new(),
                base_path: avatar_policy_base_path,
            },
            max_file_size: 0,
            chunk_size: None,
            is_default: false,
    ...
    user_profile_repo::create(
        ...
        aster_drive::entities::user_profile::ActiveModel {
            ...
            avatar_policy_id: Set(Some(avatar_policy.id)),
            avatar_key: Set(Some(format!("profile/avatar/{}/v1", user.id))),
    ...
    let err = policy_service::delete(&state, avatar_policy.id)
        .await
        .unwrap_err();
    assert_eq!(err.code(), "E005");
    assert!(
        err.message()
            .contains("uploaded avatar(s) still reference it"),
